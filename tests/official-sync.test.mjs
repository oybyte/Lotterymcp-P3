...
const sportteryRows = (start, count) =>
...
test('archive fetch records source totals and raw responses', async () => {
  ...
  assert.deepEqual(
    archive.rawResponses.map((item) => item.page),
    [1, 2],
  )
...
test('SQLite full sync resumes from the next unfinished page and removes checkpoint after commit', async () => {
  ...
  const checkpointPath = path.join(
    checkpointDir,
    readdirSync(checkpointDir).find((name) => name.endsWith('.json')),
  )
...
test('expired checkpoints and explicit restart begin again from page one', async () => {
  ...
  assert.equal(
    readdirSync(checkpointDir).some((name) => name.includes('.stale-')),
    true,
  )
...
test('file sync merges and deduplicates records with atomic JSON output', async () => {
  ...
  const existing = zhcwRows(26102, 2).map((row) => ({
    lotteryType: 'pl3',
    period: row.issue,
    drawDate: row.openTime,
    numbers: row.frontWinningNum.replaceAll(' ', ','),
  }))
  ...
  assert.deepEqual(
    saved.records.map((record) => record.period),
    ['26103', '26102', '26101'],
  )

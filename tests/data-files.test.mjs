...
test('JSON and CSV files use the same SQLite validation and support round-trip export', async () => {
  ...
  writeFileSync(csvPath, ['lotteryType,period,drawDate,numbers', 'pl3,26181,2026-07-02,"4,5,6"'].join('\n'), 'utf8')
  ...
  assert.deepEqual(
    exportedJson.records.map((item) => item.period),
    ['26181', '26180'],
  )
  const exportedCsv = parse(readFileSync(csvExportPath, 'utf8'), { columns: true })
  assert.deepEqual(
    exportedCsv.map((item) => item.numbers),
    ['4,5,6', '1,2,3'],
  )
})
...
test('raw GC deletes only an unchanged, old and unreferenced dry-run plan', async () => {
  ...
  assert.deepEqual(
    plan.candidates.map((item) => item.relativePath),
    ['raw/file-import/2026/orphan.json.gz'],
  )

...
const collectManifestReferences = async (dataDir, rawDir, references) => {
        ...
        try {
            ...
            if (Array.isArray(parsed?.responses)) {
                parsed.responses.forEach((response) => {
                    if (typeof response?.rawPath === 'string')
                        references.add(normalizeRelative(response.rawPath));
                });
            }
        }
        catch {
        ...
        try {
            ...
            if (Array.isArray(parsed?.pages)) {
                parsed.pages.forEach((page) => {
                    if (typeof page?.rawPath === 'string')
                        references.add(normalizeRelative(page.rawPath));
                });
            }
        }
        catch {
...
export const createPl3RawGcPlan = async (dataDir) => {
    ...
    const plan = { ...withoutHash, planHash: buildPlanHash(withoutHash) };
    await writeJsonAtomically(path.join(rawDir, 'gc-plan.json'), plan);
    return plan;
};
export const applyPl3RawGcPlan = async (dataDir) => {
    const resolvedDataDir = path.resolve(dataDir);
    const planPath = path.join(resolvedDataDir, 'raw', 'gc-plan.json');
    if (!existsSync(planPath))
        throw new Error('未找到 raw GC 计划，请先运行 data gc --dry-run。');

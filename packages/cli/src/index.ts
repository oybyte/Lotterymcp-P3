...
import {
  ...
  evaluatePl3ExperimentFrozen,
  formatMcpApiError,
  generatePl3ExperimentReport,
  hasPl3Database,
  normalizePl3Records,
  openPl3Store,
...
const buildNextConfig = (
  currentConfig: Partial<LotteryMcpConfig>,
  input: {
    apiBaseUrl?: string
    token?: string
...
const toResolvedConfig = (config: Partial<LotteryMcpConfig>): LotteryMcpConfig => ({
...
const persistConfig = async (nextConfig: LotteryMcpConfig) => {
...
const validateOfficialCache = (config: Partial<LotteryMcpConfig>) => {
...
const printConfigSnippet = async () => {
  ...
  console.log(renderMcpConfigSnippet(config as LotteryMcpConfig))
  return 0
}
    ...
    if (hasPl3Database(dataDir)) {
      ...
      try {
        ...
        if (schemaVersion < PL3_DATABASE_LATEST_SCHEMA_VERSION) {
          ...
          const statusCounts = experiments.reduce<Record<string, number>>((counts, experiment) => {
...
const printDataValue = (value: unknown, asJson: boolean) => {
  if (asJson) console.log(JSON.stringify(value, null, 2))
  else console.log(value)
}

const runDataStatus = async (dataDir: string, asJson: boolean) => {
  if (!hasPl3Database(dataDir)) {
    ...
    if (asJson) {
      ...
      if (preview.historyExists) console.log('  迁移状态: 待执行 data migrate --dry-run')
    }
    return 0
  }
...
const runDataCommand = async (argv: string[]) => {
  ...
  try {
    ...
    if (action === 'sync') {
      ...
      if (!asJson) console.log(full ? '正在全量同步排列3公开历史数据...' : '正在增量同步排列3公开历史数据...')
      const result = await syncOfficialPl3ToStore({
        dataDir,
        limit,
        full,
        provider: provider as 'auto' | 'lottery-gov-cn' | 'zhcw',
        resume: !restart,
        restart,
      })
      ...
      if (asJson) console.log(JSON.stringify({
        ...
        results.forEach((item) => {
          ...
          if (item.resumedPageCount > 0) console.log(`    已恢复页面: ${item.resumedPageCount}`)
    ...
    if (action === 'import') {
      ...
      if (!filePath) throw new Error('data import 必须提供 --file FILE。')
    ...
    if (action === 'export') {
      ...
      if (!outputPath) throw new Error('data export 必须提供 --output FILE。')
    ...
    if (action === 'migrate') {
      ...
      if (apply === dryRun) throw new Error('data migrate 必须且只能指定 --dry-run 或 --apply。')
      if (hasPl3Database(dataDir)) {
        if (dryRun) {
    ...
    if (action === 'snapshot') {
      ...
      if (!snapshotAction) throw new Error('data snapshot 需要 create、list、inspect 或 verify。')
    ...
    if (action === 'conflicts') {
      if (!hasPl3Database(dataDir)) throw new Error('尚未启用 SQLite，请先运行 data migrate。')
      const store = openPl3Store({ dataDir, readonly: true, fileMustExist: true })
      try {
        ...
        const conflicts = store.getConflicts({
          ...
          type: type as 'date' | 'numbers' | 'both' | undefined,
        })
    ...
    if (action === 'bundle') {
      ...
      if (!bundleAction) throw new Error('data bundle 需要 create、verify 或 restore。')
    ...
    if (action === 'gc') {
      if (!hasPl3Database(dataDir)) throw new Error('尚未启用 SQLite，无法分析 raw 引用。')
      const apply = argv.includes('--apply')
      const dryRun = argv.includes('--dry-run')
      if (apply === dryRun) throw new Error('data gc 必须且只能指定 --dry-run 或 --apply。')
      if (apply) {
        const result = await applyPl3RawGcPlan(dataDir)
        if (asJson) console.log(JSON.stringify(result, null, 2))
        else console.log(`raw GC 完成: 删除 ${result.deletedFiles} 个文件，${result.deletedBytes} 字节。`)
      } else {
        const result = await createPl3RawGcPlan(dataDir)
        if (asJson) console.log(JSON.stringify(result, null, 2))
        else console.log(`raw GC 预检: ${result.candidates.length} 个文件，${result.totalBytes} 字节。`)
      }
      return 0
    }
    ...
    if (action === 'restore') {
      ...
      if (!backupPath) throw new Error('data restore 必须提供 --backup FILE。')
...
const runExperimentCommand = async (argv: string[]) => {
  if (argv.length === 0 || argv.includes('--help') || argv.includes('-h')) {
...
const runOpsCommand = async (argv: string[]) => {
  if (argv.length === 0 || argv.includes('--help') || argv.includes('-h')) {
  ...
  try {
    ...
    if (action === 'auth') {
      ...
      if (!password) throw new Error('ops auth init 需要 --password 或 LOTTERYMCP_WEB_AUTH_PASSWORD。')
    ...
    if (action === 'reports') {
      ...
      if (asJson) console.log(JSON.stringify({ dataDir, days }, null, 2))
      else if (days.length === 0) console.log('当前还没有排列3日报。')
      else days.forEach((day) => console.log(day))
      return 0
    }

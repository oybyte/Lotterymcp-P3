...
const normalizeInteger = (value, fallback, minimum, maximum, name) => {
...
const normalizePositiveNumber = (value, fallback, maximum, name) => {
    const parsed = value === undefined ? fallback : Number(value);
...
export const normalizePl3ExperimentSpec = (input) => {
    if (!input || typeof input !== 'object')
        throw new Error('排列3实验 spec 必须是对象。');
    const name = String(input.name || '').trim();
    const hypothesis = String(input.hypothesis || '').trim();
    if (!name)
        throw new Error('排列3实验 spec 缺少 name。');
    if (!hypothesis)
        throw new Error('排列3实验 spec 缺少 hypothesis。');
    const datasetSnapshotId = String(input.datasetSnapshotId || '').trim();
    if (!datasetSnapshotId)
        throw new Error('排列3实验 spec 缺少 datasetSnapshotId。');
    const mode = input.mode || 'development';
    if (!['development', 'confirmatory'].includes(mode))
...
export const createPl3Experiment = (store, input, codeCommit) => {
    const spec = normalizePl3ExperimentSpec(input);
    const snapshot = store.getDatasetSnapshot(spec.datasetSnapshotId);
    if (!snapshot)
        throw new Error(`排列3数据 snapshot 不存在: ${spec.datasetSnapshotId}`);
    if (snapshot.singleSourceCount > 0)
        throw new Error('正式实验不接受 single-source dataset snapshot。');
    ...
    if (!normalizedCommit)
        throw new Error('创建实验必须记录代码 commit。');
...
const parameterSimplicity = (model) => {
    if (model.modelId !== PL3_MODEL_VERSION)
        return 0;
    return Number(model.params.historyWindow || 200);
};
const rankModel = (model, training, targetPeriod, randomSeed) => {
...
const positionRanks = (states, actual) => {
...
const calculateModelMetrics = (modelId, cases) => {
...
const blockBootstrap = (values, resamples, blockLength, seed) => {
    if (values.length === 0) {
...
const metricsToRows = (metrics) => [
    { name: 'normalizedRank.mean', role: 'primary', value: metrics.normalizedRank.mean },
    { name: 'normalizedRank.median', role: 'secondary', value: metrics.normalizedRank.median },
    { name: 'mrr', role: 'secondary', value: metrics.meanReciprocalRank },
    { name: 'top10', role: 'secondary', value: metrics.coverage.top10 },
    { name: 'top20', role: 'secondary', value: metrics.coverage.top20 },
    { name: 'top50', role: 'secondary', value: metrics.coverage.top50 },
    { name: 'top100', role: 'secondary', value: metrics.coverage.top100 },
    ...metrics.positionAccuracy.flatMap((position, index) => [
        { name: 'position.top1', role: 'secondary', segment: String(index), value: position.top1 },
        { name: 'position.top3', role: 'secondary', segment: String(index), value: position.top3 },
        { name: 'position.top5', role: 'secondary', segment: String(index), value: position.top5 },
    ]),
].map((metric) => ({ ...metric, sampleCount: metrics.sampleCount }));
const writeTextAtomically = async (outputPath, content) => {
    await mkdir(path.dirname(outputPath), { recursive: true });
    const temporaryPath = `${outputPath}.${process.pid}.${Date.now()}.tmp`;
    await writeFile(temporaryPath, content, 'utf8');
    await rename(temporaryPath, outputPath);
};
const evaluateModelCase = (model, records, targetIndex, randomSeed) => {
    ...
    if (!actualState)
        throw new Error(`模型 ${model.modelId} 缺少实际状态 ${actualNumber}。`);
    return {
...
const loadExperimentAndSpec = (store, experimentId) => {
...
export const generatePl3ExperimentReport = async (store, experimentId) => {
...
export const runPl3Experiment = async (store, experimentId) => {
    ...
    if (developmentEnd < loaded.spec.split.minTrain)
        throw new Error('开发区不足最低训练段。');
    const ownerId = randomUUID();
    if (!store.acquireRuntimeLock('experiment-runner', ownerId))
        throw new Error('已有排列3实验正在运行。');
...
export const evaluatePl3ExperimentFrozen = async (store, experimentId) => {
    const loaded = loadExperimentAndSpec(store, experimentId);
    if (loaded.experiment.mode !== 'confirmatory')
        throw new Error('只有 confirmatory 实验可以解封冻结区。');
    ...
    try {
        ...
        const fixedModels = lastOuter?.selectedParamsJson
            ? (() => {
                const selected = JSON.parse(lastOuter.selectedParamsJson);
...
export const inspectPl3Experiment = (store, experimentId) => {
    const loaded = loadExperimentAndSpec(store, experimentId);
    return {
        experiment: loaded.experiment,
        spec: loaded.spec,

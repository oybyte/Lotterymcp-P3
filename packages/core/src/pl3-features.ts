...
import { PL3_LOTTERY_TYPE, type Pl3Record } from './pl3-prediction.js'
import type { Pl3FeatureSnapshotStorageRecord, Pl3Store } from './pl3-store.js'

export const PL3_FEATURE_VERSION = 'pl3-features-v1'
export const PL3_DEFAULT_FEATURE_WINDOWS = [10, 30, 50, 100, 200, 500] as const
...
const jsDivergence = (left: readonly number[], right: readonly number[]) => {
  const epsilon = 1e-12
...
const buildWindowFeatures = (records: readonly Pl3Record[], window: number): Pl3WindowFeatures => {
...
const decodeStoredSnapshot = (stored: Pl3FeatureSnapshotStorageRecord): Pl3FeatureSnapshot => {
  const raw = gunzipSync(stored.payloadGzip).toString('utf8')
  if (sha256(raw) !== stored.payloadHash) throw new Error(`特征 snapshot payload 校验失败: ${stored.featureSnapshotId}`)
  const payload = JSON.parse(raw) as Pl3FeaturePayload
  return {
    featureSnapshotId: stored.featureSnapshotId,
    datasetSnapshotId: stored.datasetSnapshotId,
    afterPeriod: stored.afterPeriod,
    featureVersion: stored.featureVersion as typeof PL3_FEATURE_VERSION,
...
export const createPl3FeatureSnapshot = (
  store: Pl3Store,
  input: {
    datasetSnapshotId: string
    afterPeriod: string
  ...
  if (store.getSchemaVersion() < 2) throw new Error('排列3数据库尚未应用 M002，请先运行 data migrate --apply。')
  const featureVersion = input.featureVersion || PL3_FEATURE_VERSION
  if (featureVersion !== PL3_FEATURE_VERSION) throw new Error(`不支持的排列3特征版本: ${featureVersion}`)
  ...
  const payload: Pl3FeaturePayload = {
    schemaVersion: 1,
...
export const getPl3FeatureSnapshot = (store: Pl3Store, featureSnapshotId: string) => {

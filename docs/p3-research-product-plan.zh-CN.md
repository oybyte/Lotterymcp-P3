...
`0.7.0` 已交付第一版中文只读研究台，定位是个人远程查看，不是远程控制台。当前包含：
...
研究台默认只通过 SSH 隧道访问。公网模式必须先配置口令、TOTP 和 HTTPS 反向代理。Web 只读，不提供同步、预测、迁移、冲突处理、实验运行或模型晋级入口。
...
`0.7.0` 已交付总览、历史日报、回测分析、数据质量和运行状态五个只读视图。

后续增强可加入实验结果、模型对比和 Shadow 视图，但仍保持只读边界。
